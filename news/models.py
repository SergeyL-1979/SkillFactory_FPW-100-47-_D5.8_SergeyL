...
class CategorySubscribers(models.Model):
    ...
    subscriber_user = models.ForeignKey(User, null=True, on_delete=models.SET_NULL, verbose_name='Пользователь')
...
class Post(models.Model):
    ...
    #     data = Category.objects.filter(post_category_post=self)
    #     # Множество удобна тем, что одинаковые значения не дублируются
    #     category = set()
    #     for i in data:
    #         category.add(i.category_name)
